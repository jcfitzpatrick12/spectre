...
def import_from_receiver(receiver):
    ...
    # creat the full path to the receiver, 
    ...
    if "gr" not in dir_contents:
        raise NotADirectoryError(f"Could not find gr folder for receiver {receiver}.")
    

    if "mounts" not in dir_contents:
        raise NotADirectoryError(f"Could not find mounts folder for receiver {receiver}.")
    
    
    mounts_path = os.path.join(receiver_path, 'mounts')

    if not os.path.isdir(mounts_path):
        raise NotADirectoryError(f"{mounts_path} is not a directory.")

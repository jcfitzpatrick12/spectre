...
from spectre_server.old_services import delete
...
@delete_blueprint.route("/chunk-files", methods=["POST"])
@jsendify_response
def chunk_files():
    payload = request.get_json()
    tag = payload.get("tag")
...
@delete_blueprint.route('/fits-config', methods=["POST"])
@jsendify_response
def fits_config():
    payload = request.get_json()
    tag = payload.get("tag")
...
@delete_blueprint.route('/capture-config', methods=["POST"])
@jsendify_response
def capture_config():
    payload = request.get_json()
    tag = payload.get("tag")
